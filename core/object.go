...
type (
	...
	Fn struct {
		...
		fnExpr  *FnExpr
		env     *LocalEnv
		fn      interface{} // func(...) ... wrapper that calls Fn (which has no arguments and returns nothing; TODO: support all wrappers)
	}
	ExInfo struct {
		ArrayMap
		rt *Runtime
...
func (o GoObject) Compare(other Object) int {
	o2 := EnsureObjectIsGoObject(other, "Cannot compare GoObject and "+other.GetType().ToString(false))
