...
func initPackage(rootUnix, pkgDirUnix, nsRoot string, p *Package) {
	...
	if _, ok := PackagesInfo[pkgDirUnix]; !ok {
		PackagesInfo[pkgDirUnix] = &PackageInfo{pkgDirUnix, filepath.Base(pkgDirUnix),
			&imports.Imports{PackageName: pkgBaseName}, &imports.Imports{PackageName: pkgBaseName},
...
func FullPkgNameAsGoType(fn *FuncInfo, fullPkgName, baseTypeName string) (clType, clTypeDoc, code, doc string) {
	...
	doc = fn.Imports.Add(path.Base(fullPkgName), fullPkgName, clojureStdNs, clojureStdPath, true, fn.Pos) + "." + baseTypeName
	code = doc
	return
}
...
func processFuncDecl(gf *godb.GoFile, pkgDirUnix string, f *File, fd *FuncDecl) {
	...
	QualifiedFunctions[fullName] = &FuncInfo{fd.Name.Name, rcvrId, fnName, docName, fd, nil, fd.Type, fd.Doc, gf,
		&imports.Imports{PackageName: path.Base(pkgDirUnix)}, fd.Pos()}
}
