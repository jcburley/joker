...
Values of type `error` are now wrapped in `GoObject` instead of being converted into `String`. This allows receivers for those values to be invoked, and is
consistent with preserving the native type when not precisely implemented by a Joker type (in this case, `Error`, an abstract type). When an `error` type is
needed, the supplied expression may be `String` or `GoObject[error]`.

The `GoVar` object no longer exists. Variables are now implemented as `GoObject`s that wrap references (pointers) to the respective variables. `(deref SomeVar)` (or `@SomeVar`) still yields a snapshot of the `SomeVar` variable, and `(var-set SomeVar new-value)` on such variables continues to work for now, but is likely to be replaced with `(set! SomeVar new-value)` if and when the `set!` special function is implemented.

Similarly, `(var (. instance field))` now attempts to return a `GoObject` wrapping a reference (pointer) to the field for that instance. "Attempts" is used here because Go doesn't always ensure fields are addressable; Joker uses the `reflect` package's `.CanAddr()` method on the value of the field to determine whether this usage will work and, if not, panics. It's expected a future `gostd` version will resolve this by replacing usage of `var` and `var-set`, in such cases, with `set!`.
